...
def _rewrite_switch_control_html(devices, html_path: Path = SWITCH_CONTROL_HTML):
    ...
    if start == -1 or end == -1 or end < start:
        print(f"⚠️  Cannot auto-update switch UI (markers missing): {html_path}")
        return False
    ...
    try:
        ...
        print(f"🧩 Switch UI updated: {html_path}")
        return True
    except OSError:
        return False
...
def generate_cost_plot_per_device(data_dir: Path, output_png: Path):
    ...
    try:
        ...
        print("⚠️  matplotlib is not installed. Install with: pip install -r requirements.txt")
        return None
    ...
    if not series_by_device:
        print(f"⚠️  No plottable cost data in: {data_dir}")
        return None
    ...
    ax.set_ylabel("EUR")
    ax.set_title("Tasmota: Kosten (EUR) über Zeit")
...
def check_tasmota(ip):
    ...
    try:
        ...
        if "Tasmota" in response.text:
            print(f"🔍 Found Tasmota device: http://{ip}")
            return ip
...
def print_device_details(device_info, energy_data, state_data=None, preis_prokw=0.329):
    ...
    print(f"│ 🏠 {name:<56} │")
    print(f"├{'─' * 60}┤")
    print(f"│ 🌐 IP address:     {ip:<41} │")
    print(f"│ MAC address:      {mac:<41} │")
    print(f"│ 📱 Module:         {module:<41} │")
    print(f"│ 🔄 Version:        {version:<41} │")
    print(f"│ ⏱️  Uptime:         {uptime:<41} │")
    if power_state:
        print(f"│ 🔌 POWER:          {str(power_state):<41} │")
    print(f"│ 📶 WiFi SSID:      {wifi_ssid:<41} │")
    print(f"│ 📊 WiFi RSSI:      {wifi_rssi} dBm{'':<37} │")
    if wifi_signal_dbm is not None:
        print(f"│ 📡 Signal (State): {str(wifi_signal_dbm):<41} │")
    print(f"├{'─' * 60}┤")
    ...
    if energy_data['total'] != 'N/A':
        ...
        print(f"│ 📈 Today:          {energy_data['today']} kWh - {kosten_today:.2f} EUR{'':<22} │")
        print(f"│ 📊 Yesterday:      {energy_data['yesterday']} kWh - {kosten_yesterday:.2f} EUR{'':<22} │")
        print(f"│ 💰 Total:          {energy_data['total']} kWh - {kosten_total:.2f} EUR{'':<22} │")
        print(f"└{'─' * 60}┘")
        return kosten_total
    else:
        print(f"│ ⚠️  Energy data:     Not available{'':<30} │")
        print(f"└{'─' * 60}┘")
        return 0

def scan_network(plot: bool = True):
    print("🔍 Starting network scan for Tasmota devices...")
    ...
    if tasmota_devices:
        print(f"\n🎯 Found {len(tasmota_devices)} Tasmota device(s):")
        print("=" * 70)
        ...
        for device in tasmota_devices:
            device_count += 1
            print(f"\n📱 Device {device_count} of {len(tasmota_devices)}:")
            ...
            try:
                ...
                print(f"⚠️  Error while querying {device}: {exc}")
                continue
        ...
        print(f"\n{'=' * 70}")
        print(f"💸 TOTAL COST (all devices): {total_kosten:.2f} EUR")
        print(f"📊 Device count: {len(tasmota_devices)}")
        if len(tasmota_devices) > 0:
            print(f"💡 Average cost per device: {total_kosten/len(tasmota_devices):.2f} EUR")
        print(f"💾 Logs saved in: {DATA_DIR}")
        if plot:
...
if __name__ == "__main__":
    _ensure_utf8_stdout()
    print("🚀 Tasmota Network Scanner started")
    print("📡 Scanning local network for Tasmota devices...\n")
